...
$$;
--Обновление кортежа
--Опять же в связи с перегрузкой нужно явно указывать типы
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION update_cortege(p_order_id INT, p_user_id INT, p_status VARCHAR)
RETURNS VOID AS $$
BEGIN
    UPDATE delivery_schema.Orders
    SET status = p_status
...
CREATE OR REPLACE FUNCTION update_cortege(p_product_id INT, p_name VARCHAR(100), p_description TEXT, p_stock INT)
RETURNS VOID AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION delete_record(t_name TEXT, id INT)
RETURNS VOID AS $$
BEGIN
    ...
    ELSE
        RAISE EXCEPTION 'Table "%s" is not allowed for deletion.', p_table_name;
    END IF;
EXCEPTION WHEN OTHERS THEN
  RAISE EXCEPTION 'Ошибка при удалении записи: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
-- Заполнение таблиц
